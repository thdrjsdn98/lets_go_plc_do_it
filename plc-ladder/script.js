/* ============================================================
   지멘스 PLC 기초강좌 - PLC 핵심내용② 래더 명령어
   ⚠️ 저장소 독립성: 이 프로젝트의 모든 localStorage 키는
   반드시 STORAGE_PREFIX('plc_ladder_')로 시작합니다.
...
============================================================ */
var STORAGE_PREFIX = 'plc_ladder_';
...
function checkDailyNotify() {
    ...
    if (lastNotifyDate !== today) {
        ...
        new Notification('지멘스 PLC 기초강좌 - PLC 핵심내용② 래더 명령어 🛠️', {
...
async function exportUserData() {
    // 백업 데이터에는 이 프로젝트(plc_ladder_) 소유 데이터만 포함합니다.
...
function importUserData(event) {
    ...
    reader.onload = function (e) {
        try {
            ...
            // 복원도 plc_ladder_ 네임스페이스 안에서만 수행 (다른 프로젝트 데이터 건드리지 않음)
...
// plc_ladder_ 로 시작하는 이 프로젝트 소유 키만 선별 삭제합니다.
...
/* ===================== 퀴즈 채점 ===================== */
var unitTitles = {
    1: "01. 접점 - A접점 / B접점", 2: "02. 코일과 SET / RESET", 3: "03. 타이머 - TON / TOF / TP", 4: "04. 카운터 - CTU / CTD / CTUD", 5: "05. 비교 명령 (CMP)", 6: "06. 이동·연산 명령 (MOVE, ADD 등)", 7: "07. 데이터 타입 (Bool/Byte/Int/DInt/Real)", 8: "08. 프로그램 구조 (OB1/FC/FB/DB 활용)"
};
